<template>
    ...
    <el-dialog class="reg-review-dialog" title="Review Convention Registration Application" :visible.sync="dialogReviewTab">
...
export default {
    ...
    methods: {
        ...
        getInitialPendingMembers() {
            this.tableLoading = true
            axios.get(`/admin/registration/pending/get`)
			.then(res => {
				if(res.data) {
        ...
        deleteReg() {
            ...
            this.is_approving = true
            axios.put('/api/registration/decline/' + id, {
                headers: { "Authorization": "Bearer " + this.token }
            }).then(res => {
                if(res.data) {
                    this.getInitialPendingMembers();
                    ...
                    this.dialogReviewTab = false
                    this.$notify({
                        message: 'Selected registrant has been deleted.',
                        type: 'success',
                        customClass: 'pcr-login-error',
                        position: 'bottom-right'
                    })                                     
            ...
            .catch(err => {
                console.log(err);
				this.$notify({
					message: 'Error deleting selected registrant.',
					type: 'error',
					customClass: 'pcr-login-error',
					position: 'bottom-right'
                })
...
.reg-review-dialog .el-dialog {
    margin-top:5vh!important;
}
.reg-review-dialog .el-dialog__body {
    padding-top:10px;
}
.reg-review-dialog .el-dialog__title {
    font-weight:bold;
}
.reg-pending-table th {
    color:#131313;
}
.reg-view-image img {
    width:100%;
}
.reg-pending-table .el-table__row .cell {
